...
need to be quoted in the SQL queries. This is done by passing in the
C<quote_names> attribute into the connect_info when connecting a schema.

This will automatically determine which quotes are required for your RDBMS.
This is sugar for the C<quote_char> and C<name_sep> properties and should
set them correctly for your RDBMS.

This is generally done like this with,
...
Turning quoting on is generally a good idea, and there is even a helper
to allow you to do it without requiring explicit configuration.

See L<DBIx::Class::Helper::Schema::QuoteNames>.

Turning quoting on does mean that all terms assumed to be identifiers will
be quoted.  This means that you need to be careful with complex expressions
used on the left hand side where identifiers are assumed.  A query like this
would work without quoting on.
...
With it on the whole chunk would get encoded and so you would end up with
an error complaining that the field C<"sum(items.quantity)"> does not exist.

There are several ways to resolve this, often there is a data structure that
SQL Abstract can interpret into the function call, in this case
C<< { sum => 'items.quantity' } >> is equivalent.

Here are some examples, including using literal SQL by passing in a string
reference.
...
If you need to provide variables where there is an identifier take a look at the
L</Using SQL functions on the left hand side of a comparison>.

Also note that the opposite is possible too.  If you need to use an identier on
the right side you can use the construct C<< { -ident => 'column' } >>

For example,

 return $self->search({ col1 => { -ident => 'col2' }});
