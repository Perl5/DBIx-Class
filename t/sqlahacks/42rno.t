...
sub default_test_order {
   ...
   is_same_sql_bind(
      ...
      "(SELECT me__id, me__source, me__owner, me__title, me__price, owner__id, owner__name FROM
         (
            SELECT Q1.*, ROW_NUMBER() OVER( ) AS ROW_NUM FROM (
               SELECT me.id, me.source, me.owner, me.title, me.price, owner.id, owner.name
               FROM books me JOIN owners owner ON owner.id = me.owner WHERE ( source = ? )
            ) Q1
         ) Q2
         WHERE ROW_NUM BETWEEN 4 and 4
       )",
    [ [ source => 'Library' ] ],
  );
}
